...
class ActivityDetailsScreen extends StatefulWidget {
  ...
  const ActivityDetailsScreen({Key? key, required this.activityItemModel}) : super(key: key);
...
class _ActivityDetailsScreenState extends State<ActivityDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: CustomBody(
        appBar: CustomAppBar(title: 'check_your_all_trip'.tr,showBackButton: true,),
        body: Padding(
          ...
          child: GetBuilder<ActivityController>(builder: (activityController){
            ...
            return Column(crossAxisAlignment: CrossAxisAlignment.start,children: [
              ...
              Text('your_trip_details'.tr, style: textSemiBold.copyWith(fontSize: Dimensions.fontSizeExtraLarge,
                  color: Theme.of(context).textTheme.bodyLarge?.color)),

              Divider(color: Theme.of(context).primaryColor.withOpacity(0.2),),

              ActivityItemView(activityItemModel: widget.activityItemModel,isDetailsScreen: true,),
              const SizedBox(height: Dimensions.paddingSizeSmall,),

              const ActivityScreenMapView(),

              if(widget.activityItemModel.tripDetails!=null)
              ActivityScreenTripDetails(tripDetails: widget.activityItemModel.tripDetails!),

              if(widget.activityItemModel.tripDetails!=null)
              ActivityScreenRiderDetails(riderDetails: widget.activityItemModel.riderDetails!),
